...
### WeChat Public Account Typography

公众号文章字体偏小，适合移动端阅读。以下为微信排版专用字号规范：

| Token | Size | Weight | Line Height | Letter Spacing | Use |
|---|---|---|---|---|---|
| `{typography.wx-h1}` | 20px | 400 | 1.4 | -0.4px | 文章大标题 |
| `{typography.wx-subtitle}` | 13px | 400 | 1.4 | 0.08px | 副标题/作者信息 |
| `{typography.wx-h2}` | 17px | 400 | 1.4 | -0.2px | 章节标题 |
| `{typography.wx-h3}` | 15px | 600 | 1.5 | 0 | 小节标题 |
| `{typography.wx-body}` | 14px | 400 | 1.75 | 0.08px | 正文（默认） |
| `{typography.wx-body-strong}` | 14px | 600 | 1.75 | 0 | 正文强调 |
| `{typography.wx-quote}` | 13px | 400 | 1.6 | 0.08px | 引用块 |
| `{typography.wx-caption}` | 12px | 400 | 1.4 | 0 | 图注/次要信息 |
| `{typography.wx-label}` | 11px | 600 | 1.4 | 0.88px | 标签（大写） |

**公众号排版原则：**
- 正文 14px，行高 1.75，保证移动端可读性
- 标题层级递进：H1 20px → H2 17px → H3 15px
- 引用块 13px，与正文形成层次
- 图注 12px，不喧宾夺主
- 所有文字使用 system-ui 字体栈，确保跨平台一致性
